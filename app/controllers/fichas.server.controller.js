	...
	async.waterfall([
		...
		function (imagen, done)
		{
			...
			if (ficha._id)
			{
				...
				var upd = {codcli: ficha.codcli, nomcli: ficha.nomcli, nomtit: ficha.codtit, derecho: ficha.derecho, tramite: ficha.tramite,
					...
					interes: ficha.interes || '', estado: ficha.estado || '', deslogo: ficha.deslogo || '', finalizado: ficha.finalizado, 
					archivo: ficha.archivo || '', fechavenc: ficha.fechavenc || '', obs: ficha.obs || ''};
...
exports.descargaImagen = function(req, res, next){
	...
	fs.readFile(file, function (err, bitmap) {
		...
		res.jsonp('data:image/' + ext + ';base4,'  + data);
	});
};
